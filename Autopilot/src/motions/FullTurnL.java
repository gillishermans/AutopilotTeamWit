...
public class FullTurnL extends Motion {
	...
	private final static float X_OFFSET = 754;
	private final static float Z_OFFSET = 576; //540
	
	
	public FullTurnL(float heading) {
...
//			if ((desiredHeading < 0 && inputs.getHeading() > 0)) {
...
//			}
			...
			//else if (Math.abs(inputs.getRoll()) > maxRoll) {
			outputRoll = pidMaxRoll.getOutput( maxRoll,inputs.getRoll(), time);
			//}
			//else {
//				if (desiredHeading < 0 && inputs.getHeading() > 0) {
...
//				else 						  	   outputRoll = pidDraai.getOutput(desiredHeading, inputs.getHeading(), time);
			...
			if (Math.abs(desiredHeading - inputs.getHeading()) > Math.PI/720) {
				...
				System.out.println("Actual FullTurnL: " + inputs.getX() + " " + inputs.getZ());
				this.done = true;
			}
	...
	public Vector nextPos(AutopilotInputs inputs) {
		...
		System.out.print("Prediction FullTurnL: ");
		output.print();
		return output;
	}
